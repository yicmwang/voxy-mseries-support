...
bool isQuadEmpty(Quad quad) {
    // ivec3(0), not ivec2(0): Quad gained the baked-faceModelData component, and GLSL will not compare
    // vectors of different sizes. A quad with zero faceModelData is still "empty" by this test only if
    // every component is zero, which is what the mesher's zero-filled range looks like.
    return all(equal(quad, ivec3(0)));
}