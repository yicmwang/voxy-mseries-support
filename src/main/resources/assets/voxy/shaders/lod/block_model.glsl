struct BlockModel {
    ...
    uint colourTint;
    uint customId;
    // 28 bytes of padding, which no one writes: ModelFactory fills faceData (0..23), flagsA (24),
    // colourTint (28) and customId (32) and stops. It was worth testing whether removing it helped the
    // vertex stage, because the model load costs 14 % of the LOD pass's gpu and the array stride is
    // where the bytes are. It does not: `VOXY_MODEL_TIGHT` took the stride from 64 to 36 and moved gpu
    // by +0.8 %, i.e. nothing, while removing the load entirely moved it -14.3 %. That is how the cost
    // was identified as the dependent load's latency rather than bandwidth, and the switch was deleted
    // with this note as what remains of it. optimisation.MD §33.
    uint _pad[7];
};
