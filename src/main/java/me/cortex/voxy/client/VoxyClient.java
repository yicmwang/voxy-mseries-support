...
public class VoxyClient implements ClientModInitializer {
    ...
    public static void initVoxyClient() {
        ...
        // Query the active render backend rather than GL capabilities directly:
        // on macOS Apple Silicon the GL driver tops out at 4.1 (no compute, no
        // indirect draws), which would fail this check even though the Metal
        // backend supports both. MetalRenderBackend reports compute and indirect
        // as available.
        RenderBackend backend = RenderBackendFactory.get();
        // Metal is the only backend — the type is no longer a runtime question.
        Logger.info("Render backend: METAL"
        ...
        // indirectParameters=true, the render path was raw GL back when this gate
        // was written and would abort the JVM. The migration to the encoder API is
        // done, but the gate stays: with the flag off the mixins find
        // VoxyRenderSystem == null and Sodium's chunk render runs unmodified — the
        // user sees normal close-distance MC + Sodium rendering instead of a blue
        // screen. build.gradle defaults the flag ON for the dev client.
        ...
        if (systemSupported) {
            if (forceMetal) {
                Logger.info("[VOXY_FORCE_METAL] Voxy enabled on METAL"
                ...
                Logger.warn("[M9 TRANSITIONAL] Voxy disabled on METAL"
