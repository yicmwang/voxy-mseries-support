...
public class MDICSectionRenderer extends AbstractSectionRenderer<MDICViewport, BasicSectionGeometryData> {
    ...
    private void traceCorners(MDICViewport viewport, long indirectOffset, int n) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            // BlockModel = { uint faceData[6]; ... } at MODEL_SIZE bytes per entry. Read through the
            // constant rather than a literal 64, so the CPU stride has one source and cannot drift from
            // the shader's struct layout -- a hardcoded stride that disagreed would read another
            // model's face data and still return a plausible-looking number.
